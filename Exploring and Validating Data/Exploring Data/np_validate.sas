...
* Step a. - First 20 Rows *;
proc print data = pg1.np_summary (obs = 20);
run;


* Step b. - First 20 Rows and Specific Columns *;
proc print data = pg1.np_summary (obs = 20);
	var Reg Type ParkName DayVisits TentCampers RVCampers;
run;
/* Yes, there are inconsistencies.
I noticed that "Type" has values of PRE and PRESERVE, which seem redundant.
They likely could be consolidated into one common value. */;
...
No, that value is not unexpected, as the column TentCampers in step b. had several 0's. */;


* Step d. - Enhanced Summary Statistics *;

proc univariate data = pg1.np_summary;
	var DayVisits TentCampers RVCampers;
run;
* No, there are no negative values for any of the columns. *;
...
proc freq data = pg1.np_summary;
	tables Reg Type;
run;
*/ No, there are no lowercase codes. Yes, there are codes that occur only once in the table.
Those codes occurring only once are NC for Reg and NPRE and RIVERWAYS for Type. */;